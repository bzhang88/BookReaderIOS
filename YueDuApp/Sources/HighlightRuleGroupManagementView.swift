...
/// Direct 高亮规则分组 CRUD -- same gap `ReplaceRuleGroupManagementView` already fixed for
/// 净化规则 groups, just for highlight rules: until now a group only ever came into existence as a
/// side effect of typing a name into a rule's own edit form, and vanished the instant no rule
/// referenced it anymore. Backed by `highlightRuleGroupStore` for the "can exist with zero rules"
/// part, merged with whatever group names are still in live use across `highlightRuleStore` so this
/// shows the complete picture regardless of whether a name was ever explicitly registered.
struct HighlightRuleGroupManagementView: View {
    ...
    private func reload() async {
        let registered = (try? await env.highlightRuleGroupStore.all()) ?? []
        let rules = (try? await env.highlightRuleStore.all()) ?? []
    ...
    private func createGroup() async {
        ...
        try? await env.highlightRuleGroupStore.add(trimmed)
        newGroupName = ""
        await reload()
    }
    ...
    private func rename(_ oldName: String, to newName: String) async {
        ...
        try? await env.highlightRuleGroupStore.rename(oldName, to: trimmed)
        let rules = (try? await env.highlightRuleStore.all()) ?? []
        let updates = rules.filter { $0.group == oldName }.reduce(into: [String: String?]()) { result, rule in
            result[rule.id] = trimmed
        }
        if !updates.isEmpty {
            try? await env.highlightRuleStore.setGroups(updates)
        }
        await reload()
    }
    ...
    private func deleteGroup(_ name: String) async {
        try? await env.highlightRuleGroupStore.remove(name)
        let rules = (try? await env.highlightRuleStore.all()) ?? []
